...
public sealed class GumletService : IGumletService
{
    private readonly HttpClient _client;
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly GumletSettings _settings;

    public GumletService(IOptionsMonitor<GumletSettings> options, IHttpClientFactory httpClientFactory)
    {
        _settings = options.CurrentValue;
        _httpClientFactory = httpClientFactory;
        _client = httpClientFactory.CreateClient(nameof(GumletService));
    ...
    public async Task<GumletAsset?> GetAsset(string assetId, CancellationToken cancellationToken = default)
    {
        using var response = await _client.GetAsync($"v1/video/assets/{assetId}", cancellationToken);
        if (response.StatusCode == System.Net.HttpStatusCode.NotFound) return null;
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<GumletAsset>(cancellationToken);
    }

    public async Task<GumletAsset> CreateAsset(byte[] bytes, string? title = null, string? creatorId = null, string? externalId = null, CancellationToken cancellationToken = default)
    {
        ...
        var asset = await createResponse.Content.ReadFromJsonAsync<GumletAsset>(cancellationToken)
            ?? throw new InvalidOperationException("Gumlet returned an empty asset response.");
        ...
        using var uploadResponse = await _httpClientFactory.CreateClient("GumletUpload").PutAsync(asset.UploadUrl, content, cancellationToken);
        uploadResponse.EnsureSuccessStatusCode();
        return await GetAsset(asset.AssetId, cancellationToken)
            ?? throw new InvalidOperationException($"Gumlet asset '{asset.AssetId}' could not be read after upload.");
    }
